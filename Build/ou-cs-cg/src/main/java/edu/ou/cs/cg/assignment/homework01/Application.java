	...
	implements GLEventListener, Runnable
{
	...
	private void	update(GLAutoDrawable drawable)
	{
		...
		m = (int)Math.floor(m * 1.08) + 1; //Increase point count
	...
	private void	render(GLAutoDrawable drawable)
	{
		...
		// Draw the scene
		if (k < 115) 			//First iteration of Tinkerbell map
			drawSomething(gl);
		else if (k >= 115 & k < 223) 			//Second iteration of Tinkerbell map
			drawSomethingLine(gl);
		else if (k >= 223 & k < 334)		//Third iteration of Tinkerbell map
			drawLineWithY(gl);
		else if (k >= 334 & k < 445 ) 		//Fourth iteration of Tinkerbell map
			drawPointWithY(gl);
		else
			k = 1;				//Infinite loop of all 4 modes
	...
	/*
		Draw Tinkerbell map with points
	*/
	private void	drawSomething(GL2 gl)
	{
		...
		for (int i = 0; i < m; i++){
			...
			double xn1 = ((x*x) - (y*y)) + (a*x) + (b*y);
			double yn1 = (2*x*y) + (c*x) + (d*y);

			x = xn1;
			y = yn1;

			gl.glColor3f(1.0f, 1.0f, 1.0f);	// Draw in white
			gl.glVertex2d( x /2.0, y/2.0 );			// Scale map to be larger
		}
	...
	/*
		Draw Tinkerbell map with line strips
	*/
	private void drawSomethingLine(GL2 gl){

		gl.glBegin(GL.GL_LINE_STRIP);			// Start specifying line strips
		...
		gl.glEnd();						// Stop specifying line strips


	}

	/*
		Draw Tinkerbell map with line strips and instead of plotting 
		(xn+1, yn+1), plot (yn, yn+1)
	*/
	private void drawLineWithY(GL2 gl){
		...
		for (int i = 0; i < m; i++){
			...
			gl.glColor3f(1.0f, 1.0f, 1.0f);	// Draw in white
			gl.glVertex2d( yn1 /2.0, y/2.0 );			// Scale map to be larger
		}
	...
	/*
		Draw Tinkerbell map with points and instead of plotting 
		(xn+1, yn+1), plot (yn, yn+1)
	*/

	private void drawPointWithY(GL2 gl){
